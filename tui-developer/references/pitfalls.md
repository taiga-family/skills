...
v5 instances live in [v5/facts.md](v5/facts.md), but always prefer the live source.
...
7. **Missing app-level prerequisites** (root wrapper, root provider, icon assets) when nothing renders or
   portalled UI never appears — that is [`tui-setup`](../../tui-setup/SKILL.md), not a code fix.
...
Durable red flags — any hit is a smell to re-check against the live source (concrete v5 patterns in
